...
module PtOnlineMigration
	...
	class PTCommandBuilder
		...
		def initialize(table_name, options, execute)
			...
			@cmd_prefix = ["pt-online-schema-change h=#{options.delete(:host)},u=#{options.delete(:username)},p=#{options.delete(:password)},D=#{options.delete(:database)},t=#{table_name} #{execute_clause}"]
			@pt_options = options
			@alter_statements = []
		end
